...
class OperNode(object):
    ...
    def is_Identical( rn1, rn2 ):
        if (rn1 == None and rn2 == None) : # Ambos vazios -> ok
            ...
            if (rn1._data == rn2._data and OperNode.is_Identical(rn1.left, rn2.left) and OperNode.is_Identical(rn1.right, rn2.right)) : 
                return True
            else: 
                return False
    ...
    def create_operation( self, operation, larg, rarg ):
        if operation in '+-*/':
            self.left = OperNode( larg )
            self.right = OperNode( rarg )
    ...
    def merge(self):
        if( self.is_operation() ):
            #Merge para os filhos
            if( self.left is not None):
                self._data = self.left.merge()
            if( self.right is not None):
                self.right._data = self.right.merge()
            #Identifica e opera
            if self._data == '+':
                self._data = self.left._data + self.right._data
            elif self._data == '-':
                self._data = self.left._data - self.right._data
            elif self._data == '*':
                self._data = self.left._data * self.right._data
                ...
                #Nada a fazer afora returnar o dado armazenado
                pass
            #end_operation
        #Retorna o valor agregado
        return self._data
    ...
    def insert_value(self, value):
        ...
        if self._data == value:
            ...
            if self.left:
                ...
                self.left = OperNode(value)
                return True
        else:
            if self.right:
                return self.right.insert_value(value)
            else:
                self.right = OperNode(value)
                return True
...
class OperBinaryTree(object):
    ...
    def is_similar( self, tree ):
        ...
        rn1 = self.root_node
        rn2 = tree.root_node
        return OperNode.is_Identical( rn1, rn2 )
