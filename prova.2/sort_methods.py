...
def __merge(left, right, ascending, compare):
    """Une as partes separadas de volta
    Método auxiliar para rotina merge_sort()"""
    ret = []
    ptl = 0
    ptr = 0
...
def merge_sort(arr, ascending=True, compare=lambda x, y, asc: (x > y, x < y)[asc]):
    ...
    if len(arr) < 2:
        ...
        return __merge(larr, rarr, ascending, compare)
...
def timsort(data, ascending=True):
    """Ordenação através do método tim sort"""
    # buscar incremento pela referência https://github.com/reingart/pypy/blob/master/rpython/rlib/listsort.py
    if ascending:
    ...
    for item in runs:  # para todo elemento em runs, agrega usando insert_sort
        sorted_runs.append(__tim_insertion_sort(item))
...
def tim_merge(left, right):
    """Método auxiliar para unir dois arranjos comparando os elementos por vez [1, 2, 3, 4, 5, 6]"""
    if not left:
        return right
    if not right:
        return left
...
def __tim_insertion_sort(the_array):
    """Método auxiliar da ordenação tim_sort()"""
    l = len(the_array)
    for index in range(1, l):
        value = the_array[index]
        pos = __tim_binary_search(the_array, value, 0, index - 1)
        the_array = the_array[:pos] + [value] + the_array[pos:index] + the_array[index+1:]
    return the_array


def __tim_binary_search(the_array, item, start, end):
    """Método auxiliar para rotina tim_sort()"""
    if start == end:
    ...
    if the_array[mid] < item:
        return __tim_binary_search(the_array, item, mid + 1, end)
    elif the_array[mid] > item:
        return __tim_binary_search(the_array, item, start, mid - 1)
    else:
        return mid
